...
const groupedSpecialSpells = computed(() => {
  const allFeatures = [
    ...(racialTraits.value || []),

    ...(baseClassFeatures.value || []),

    ...(subclassFeatures.value || []),

    ...(character.value.chosenFeats || []),
  ]

  const specialSpells = allFeatures

    .filter((feature) => feature.spells && feature.spells.length > 0) // Prende solo i privilegi con incantesimi

    .flatMap((feature) => feature.spells) // Estrae tutti gli incantesimi in una singola lista

  // Ora raggruppa gli incantesimi per la loro fonte (es. "Giuramento", "Talento", etc.)

  return specialSpells.reduce((groups, spell) => {
    const key = spell.source || 'Sconosciuta'

    if (!groups[key]) {
      groups[key] = []
    }

    groups[key].push(spell.name)
...
// Gestione concentrazione globale

const concentration = ref({ active: false, spell: null, level: null })

// In CharacterSheet.vue, trova questa funzione e sostituiscila

function addSpellFromGrimoire(spellData) {
  const { level, name } = spellData

  const spellList = character.value.spellcasting.spellbook[level]

  if (spellList && !spellList.some((spell) => spell.name === name)) {
    // Cerca l'incantesimo completo nel nostro database di dndSpells

    const fullSpellDetails = dndSpells.find((s) => s.name === name)

    if (fullSpellDetails) {
      // Salva l'intero oggetto dell'incantesimo, non solo il nome

      spellList.push({
        ...fullSpellDetails, // Copia tutti i dati (inclusa la concentrazione)

        prepared: false, // Imposta come non preparato di default
      })

      toast.success(`"${name}" aggiunto al grimorio!`)
    } else {
      toast.error(`Dettagli non trovati per l'incantesimo "${name}".`)
    }
  }
}

function setConcentration(level, spell) {
  // Questi log sono nel posto giusto e non causano errori

  console.log('--- Pulsante C Cliccato ---', spell.name)

  if (concentration.value.active && concentration.value.spell === spell.name) {
    concentration.value = { active: false, spell: null, level: null }
  } else {
    concentration.value = { active: true, spell: spell.name, level: level }
  }

  console.log('Nuovo stato di concentration:', concentration.value)
}
...
// In CharacterSheet.vue, aggiungi questa nuova funzione
  ...
  <div class="character-sheet">
    ...
    <section class="box">
      ...
      <div v-if="isCombatOpen" class="section-content">
        <div class="combat-grid">
          ...
          <div class="grid-item death-saves-box full-width">
            <label>Tiri Salvezza contro Morte (CD 10)</label>

            <div class="saves-tracker">
              <div class="successes">
                <span>Successi</span>
                ...
                <div class="checkbox-group">
                  <input
                    type="checkbox"
                    v-model="character.combat.deathSaves.successes"
                    :true-value="1"
                    :false-value="0"
                  />
              ...
              <div class="failures">
                ...
                <div class="checkbox-group">
                  <input
                    type="checkbox"
                    v-model="character.combat.deathSaves.failures"
                    :true-value="1"
                    :false-value="0"
                  />
    ...
    <section class="box">
      <div class="section-header" @click="isResourcesOpen = !isResourcesOpen">
        ...
        <div class="rest-buttons">
          <button @click.stop="doShortRest" class="reset-btn short-rest">Riposo Breve</button>
    ...
    <section class="box">
      ...
      <div v-if="isFeaturesOpen" class="section-content">
        <div class="add-feat-section">
          <select v-model="featToAdd">
            <option :value="null" disabled>-- Seleziona un talento --</option>

            <option v-for="feat in availableFeats" :key="feat.name" :value="feat.name">
              {{ feat.name }}
            </option>
          </select>

          <button @click="addFeat" class="add-btn small">+ Aggiungi Talento</button>
        </div>

        <div class="chosen-feats-list">
          <div v-for="(feat, index) in character.chosenFeats" :key="feat.name" class="feat-item">
            <span>{{ feat.name }}</span>

            <button @click="removeFeat(index)" class="remove-btn small">-</button>
          </div>

          <p v-if="!character.chosenFeats || character.chosenFeats.length === 0" class="no-feats">
    ...
    <section class="box">
      ...
      <div v-if="isFeaturesOpen" class="section-content">
        <FeaturesAndTraits
          ...
          :chosen-feats="character.chosenFeats"
    ...
    <section class="box">
      ...
      <div v-if="isSpellsOpen" class="section-content">
        ...
        <div v-if="concentration.active" class="concentration-indicator-main">
          🧠 <strong>Concentrazione Attiva:</strong>

          <span>{{ concentration.spell }} (Liv. {{ concentration.level }})</span>

          <button
            @click="makeCheck(`Tiro Salvezza su Costituzione`, savingThrowModifiers.constitution)"
          >
            TS su COS
          </button>
        </div>

        <div class="spell-slots-grid"></div>
              ...
              <li
                ...
                :class="{
                  'concentration-active':
                    concentration.active && concentration.spell === spell.name,
                }"
              >
                <div class="spell-card-header">
                  ...
                  <button
                    ...
                    @click="setConcentration(level, spell)"
                    :class="[
                      'concentration-btn',

                      { active: concentration.active && concentration.spell === spell.name },
                    ]"
                    :title="
                      concentration.active && concentration.spell === spell.name
                    ...
                    "
                  >
                    <span style="font-size: 1.5em; font-weight: bold; line-height: 1">C</span>
                  </button>
                ...
                <div
                  v-if="concentration.active && concentration.spell === spell.name"
                  class="concentration-indicator"
                >
                  <span style="font-size: 1.5em; font-weight: bold; line-height: 1">C</span>

                  <strong>Concentrazione attiva su questo incantesimo</strong>
                </div>
                ...
                <div class="spell-card-details">
                  <p>
                    ...
                    {{ spell.casting_time || getSpellDetails(spell.name)?.casting_time }}
                  </p>

                  <p>
                    <strong>Raggio:</strong> {{ spell.range || getSpellDetails(spell.name)?.range }}
                  </p>

                  <p>
                    <strong>Bersaglio:</strong>

                    <span class="bersaglio">{{
                      spell.target ? spell.target : getSpellDetails(spell.name)?.target || '—'
                    }}</span>
                  </p>
                  ...
                  <p>
                    ...
                    {{
                      (spell.components && spell.components.join
                        ...
                        : spell.components) || getSpellDetails(spell.name)?.components?.join(', ')
                    }}
                  </p>

                  <p
                    v-if="spell.material || getSpellDetails(spell.name)?.material"
                    class="material-component"
                  >
                    <strong style="color: #b30000">Materiale:</strong>

                    <span class="materiale">{{
                      spell.material || getSpellDetails(spell.name)?.material
                    }}</span>
                  </p>
                  ...
                  <p>
                    ...
                    {{ spell.duration || getSpellDetails(spell.name)?.duration }}
                  </p>

                  <p class="spell-description-summary">
                    {{ spell.description || getSpellDetails(spell.name)?.description || '' }}
                  </p>
                </div>
              </li>
            </ul>
          ...
          <div v-for="level in 9" :key="level" class="spell-level-column">
            ...
            <ul class="spell-list">
              ...
              >
                ...
                <div v-if="concentration.active" class="concentration-indicator">
                  <span>🧠 <strong>Concentrazione attiva:</strong> {{ concentration.spell }}</span>
                </div>
              </li>
            </ul>
          </div>
    ...
    <section class="box">
      <div class="section-header" @click="isSpecialSpellsOpen = !isSpecialSpellsOpen">
        <h3>Incantesimi Speciali</h3>

        <span class="toggle-icon">{{ isSpecialSpellsOpen ? '▼' : '▶' }}</span>
      </div>

      <div v-if="isSpecialSpellsOpen" class="section-content">
        <div v-if="Object.keys(groupedSpecialSpells).length > 0">
          <div
            v-for="(spells, source) in groupedSpecialSpells"
            :key="source"
            class="special-spells-group"
          >
            <h4>{{ source }}</h4>

            <ul class="special-spells-list">
              <li v-for="spellName in spells" :key="spellName">{{ spellName }}</li>
            </ul>
          </div>
        </div>

        <p v-else class="placeholder">
          Nessun incantesimo da privilegi di classe, tratti razziali o talenti.
        </p>
      </div>
    </section>
