...
// Passiamo l'intero oggetto props al composable, che ora gestirà la reattività
const { getToHitBonus, getDamageBonus, makeAttackRoll, makeDamageRoll, getWeaponDamageDie } =
  useCombat(props)
...
<template>
  <div class="combat-section">
    ...
    <div class="add-weapon-section">
      <select v-model="weaponToAdd">
        ...
        <optgroup v-for="(group, category) in groupedWeapons" :label="category">
          <option v-for="weapon in group" :key="weapon.name" :value="weapon.name">
            {{ weapon.name }}
          </option>
        </optgroup>
      </select>
      <button @click="addWeapon" class="add-btn small">+ Aggiungi</button>
      <button @click="isCustomWeaponModalOpen = true" class="add-btn small manual-btn">
        Manuale
      </button>
    </div>
    ...
    <div class="weapon-list">
          ...
          <div class="weapon-main-line">
            ...
            <div class="enhancement-wrapper">
              +<input
                ...
                v-model.number="weapon.enhancementBonus"
              />
            </div>
            <label
              v-if="weapon.properties.some((p) => p.startsWith('versatile'))"
              class="versatile-label"
            >
      ...
      <div v-if="!character.equipment.weapons.length" class="no-weapons-placeholder">
        Nessuna arma equipaggiata.
      </div>
    </div>

    <div
      v-if="isCustomWeaponModalOpen"
      class="modal-overlay"
      @click.self="isCustomWeaponModalOpen = false"
    >
      <div class="modal-content">
        ...
        <div class="form-grid">
          <div class="grid-item full-width">
            ...
            <input
              type="text"
              v-model="newCustomWeapon.name"
              placeholder="Es. Spada Lunga Elfica"
            />
          </div>
          <div class="grid-item">
          ...
          <div class="grid-item full-width">
            ...
            <input
              ...
              @input="
                newCustomWeapon.properties = $event.target.value.split(',').map((p) => p.trim())
              "
