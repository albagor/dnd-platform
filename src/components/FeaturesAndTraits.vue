...
const modalTitle = computed(() => {
  switch (modalType.value) {
    case 'trait': return 'Aggiungi Tratto Razziale';
    case 'feature': return 'Aggiungi Privilegio di Classe';
    case 'armor': return 'Aggiungi Competenza Armatura';
    case 'weapon': return 'Aggiungi Competenza Arma';
    case 'tool': return 'Aggiungi Competenza Strumento';
    case 'language': return 'Aggiungi Linguaggio';
    default: return 'Aggiungi Voce';
  }
});
</script>
    ...
    <div class="main-grid">
      <div class="column">
        ...
        <div class="sub-section">
          ...
          <p class="prof-list">{{ character.proficiencies.manualLanguages.join(', ') || 'Nessuno' }}</p>
        </div>
      </div>
      ...
      <div class="column">
        <div class="sub-section">
          ...
          <ul class="feature-list">
            <li v-for="trait in racialTraits" :key="trait.name"><strong>{{ trait.name }}:</strong> {{ trait.description }}</li>
        ...
        <div class="sub-section">
          ...
          <ul class="feature-list">
            <li v-for="feature in baseClassFeatures" :key="feature.name">
              ...
              <div v-if="feature.name === 'Stile di Combattimento' && dndFeatureChoices[feature.name]" class="choice-box">
                <select v-model="character.combat.fightingStyles[0]">
                  <option :value="undefined" disabled>-- Scegli uno stile --</option>
                  <option v-for="style in dndFeatureChoices['Stile di Combattimento'].options" :key="style.name" :value="style.name">
                    {{ style.name }}
                  </option>
                </select>
              </div>
            </li>
            <li v-if="!baseClassFeatures.length" class="placeholder">Nessun privilegio di classe base.</li>
          </ul>
        </div>
      </div>
    </div>
    ...
    <div v-if="isModalOpen" class="modal-overlay" @click.self="isModalOpen = false">
      <div class="modal-content">
        ...
        <div v-if="modalType === 'trait' || modalType === 'feature'">
          <div class="form-item"><label>Nome</label><input type="text" v-model="newEntry.name"></div>
          <div class="form-item"><label>Descrizione</label><textarea v-model="newEntry.description" rows="3"></textarea></div>
        </div>
        <div v-else>
          <div class="form-item"><label>Testo</label><input type="text" v-model="newEntry.text"></div>
        </div>
...
<style scoped>
.main-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; align-items: flex-start; }
.column { display: flex; flex-direction: column; gap: 1.5rem; }
.sub-section {}
.column-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #ccc; margin-bottom: 0.5rem; padding-bottom: 0.5rem; }
h4 { font-family: serif; text-transform: uppercase; letter-spacing: 1px; margin: 0; }
.add-btn-small { background-color: #2c3e50; color: white; border: none; border-radius: 50%; width: 24px; height: 24px; font-size: 1.2em; cursor: pointer; line-height: 24px; flex-shrink: 0; }
.prof-list { font-size: 0.9em; color: #333; margin: 0; }
.feature-list { list-style: none; padding: 0; margin: 0; }
.feature-list li { margin-bottom: 1rem; font-size: 0.9em; line-height: 1.4; }
.feature-list li strong { color: #333; }
.placeholder { color: #888; font-style: italic; }
.choice-box { margin-top: 0.5rem; padding: 0.5rem; background-color: #f0f0f0; border-radius: 4px; border: 1px solid #ddd; }
.choice-box select { width: 100%; }
.subclass-spotlight { margin-top: 2rem; padding-top: 1.5rem; border-top: 2px solid #888; }
.subclass-block h4 { color: #8e44ad; }
/* Stili Modale */
.modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.6); display: flex; justify-content: center; align-items: center; z-index: 1000; }
.modal-content { background-color: white; color: #000; padding: 25px; border-radius: 8px; box-shadow: 0 5px 15px rgba(0,0,0,0.3); width: 90%; max-width: 500px; }
.modal-content h3 { margin-top: 0; text-align: center; }
.form-item { margin-bottom: 1rem; }
.form-item label { display: block; margin-bottom: 0.25rem; font-size: 0.8em; font-weight: bold; }
.form-item input, .form-item textarea { width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 1em; }
